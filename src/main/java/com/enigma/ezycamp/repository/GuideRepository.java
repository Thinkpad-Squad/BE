...
public interface GuideRepository extends JpaRepository<Guide, String> {
    ...
    @Query(value = "select g.* from m_guide as g join m_user_account as ua on ua.id = g.user_account_id where g.guide_name like :name and ua.is_enable = true", nativeQuery = true)
