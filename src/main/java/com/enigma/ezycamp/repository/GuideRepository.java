...
public interface GuideRepository extends JpaRepository<Guide, String> {
    ...
    @Query(value = "select g.* from m_guide as g join m_user_account as ua on ua.id = g.user_account_id join m_location as l on l.id = g.location_id where l.name like :location and ua.is_enable = true", nativeQuery = true)
